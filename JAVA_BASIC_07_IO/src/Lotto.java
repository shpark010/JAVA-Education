...
public class Lotto {
	...
	public int menu() {
		...
		System.out.println("1. 로또 번호 생성 2. 로또 번호 출력 3. 종료 및 로또 번호 파일 리셋");
		...
		try {
			...
			switch(selectMenu) {
				case 1 : 
					...
					break;
				case 2 :
					System.out.println("지금까지 생성되었던 로또 번호를 출력합니다.");
					printLotto();
					break;
				case 3 : 
					System.out.println("생성되었던 로또 번호 txt 파일을 리셋 합니다.");
	...
	public void resetLotto() {
		try {
			fw = new FileWriter("LottoNum.txt",false);
	...
	public void writeLotto() {
		try {
			fw = new FileWriter("LottoNum.txt", true);
			bw = new BufferedWriter(fw);
			bw.write("로또번호 : ");
			for (int i = 0; i < lotto.length; i++) {	//로또 번호 출력
				bw.write("[" + lotto[i] + "]");
			...
			bw.write(" " + (String)now.format(dtf));
			bw.write("\n");
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public void printLotto() {
		try {
			fr = new FileReader("LottoNum.txt");
